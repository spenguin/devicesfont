    ...
    elseif( isset( $_POST['subFontName'] ) )
    {
        ...
        else
        {
            ...
            else
            {
                ...
                \Fontseller\Admin\setFamilyStandard( $familyId );
...
function setFontFamily()
{
    ...
    $classes    = \Fontseller\Admin\getVariants( 'classification' ); 
    $standards  = \Fontseller\Admin\getVariants( 'standard' ); 
...
function checkFamilyName( $guess )
{
    ...
    if( !empty( $res ) )
    {
        ...
        foreach( $res as $r )
        {
            ...
            $o[$r->ID]  = [
                'title'             => $r->post_title,
                'classifications'   => \Fontseller\Admin\getPostClassifications( $r->ID, $parent->term_id )
            ];
        }
...
function writeFonts( $parentId )
{
    ...
    {  
        ...
        {
            \Fontseller\Admin\deleteDirectory( FS_UPLOAD . 'upload/' . $f );
        }
        else
        {
            $o[]    = $f;
...
function deleteDirectories( $dirs )
{
    foreach( $dirs as $d )
    {
        \Fontseller\Admin\deleteDirectory( FS_UPLOAD . 'upload/' . $d );
    }
}
...
{    var_dump( 'target: ' .  $target );
    if( is_dir( $target ) )
    {
        ...
        {   
            \Fontseller\Admin\delete_files( $file );      
        }
...
/**
 * Get all Classifications for specified Parent
 */
function getVariants( $parentSlug )
{
    $parent     = get_term_by( 'slug', $parentSlug, 'variant' ); 
...
/**
 * Set the Standard for the new Font Family
 */
function setFamilyStandard( $familyId )
{
    if( !empty( $_POST['standard'] ) )
    {
        wp_set_post_terms( $familyId, $_POST['standard'], 'variant' );
    }
}
?> 